   ...
   switch(gctl_t.response_wifi_signal_label){
	...
	case OPEN_ON_ITEM:
        if(wifi_link_net_state()==1){ //WT.EDIT 2025.03.27
			...
			//LL_mDelay(10);//tx_thread_sleep(5);//HAL_Delay(5);
			MqttData_Publish_SetOpen(1);  
			//LL_mDelay(200);//HAL_Delay(100);//LL_mDelay(1000);//HAL_Delay(100);
	  ...
	  case FAN_ITEM:
	    if(gpro_t.gpower_on ==power_on){
			...
			//tx_thread_sleep(200);//LL_mDelay(1000);//HAL_Delay(350);
    		SendWifiData_To_PanelWindSpeed(gctl_t.set_wind_speed_value);
			//LL_mDelay(10);//HAL_Delay(10);
